...
traditional, indicator-driven rules for this. The problem is that such a rule is static: it maps a
fixed set of indicators to a fixed response and does not adjust when the market narrative or the
surrounding context changes. We want to build an inherently dynamic alternative, one that can take
...
but it has to stay auditable, explainable, and under human oversight, with the final call left to a
...
Together with an asset-management partner, we are building a reusable agentic platform for finance
and proving it on one concrete use case: dynamic, sentiment-aware recommendations for a portfolio's
bond and stock split, delivered as an investment-committee memo with a full audit trail behind it.
The platform handles the orchestration, workflow state, tool use, validation, and human approval
steps. On top of it runs the allocation workflow, which integrates the firm's existing quantitative
...
at decision quality together with how well each recommendation can be traced back to its sources.
...
covering the architecture, integration, evaluation, limitations, and governance.
